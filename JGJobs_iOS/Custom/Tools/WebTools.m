    ...
    [manager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (aesKey && (aesKey.length > 0) && [responseObject isKindOfClass:[NSData class]]){
            ...
            if(err){
                ...
                if (code == 999) {
                    [JGTools alertOnlySureActionWithTitle:@"提示" DetailTitle:@"您的账号在其他地方登录" AlertControllerStyle:1 PresentBy:[UIApplication sharedApplication].keyWindow.rootViewController method:^{
                        [[BizPayManager shareManager] loginOut];
                    }];
                }
            }
        ...
        NSLog(@"\nJGTicket --- 错误信息:%@", error);
//        [BZPTools alertOnlySureActionWithTitle:@"提示" DetailTitle:@"网络错误,请重试" AlertControllerStyle:1 PresentBy:[UIApplication sharedApplication].keyWindow.rootViewController method:^{
//
//        }];
    }];
}
