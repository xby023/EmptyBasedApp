...
/**
 ...
 @param originalvalue 原数
 @param decimals 精度
 @return 编码后数据
 */
+ (NSString *)codingWithOriginalValue:(NSString *)originalvalue Decimals:(NSInteger)decimals{
    
    //判断是否有小数点
    if ([originalvalue containsString:@"."]) {
        //计算需要拼接0的个数
        NSArray *array = [originalvalue componentsSeparatedByString:@"."];
        NSString *decimalString = array[1];
        NSInteger decimalLength = decimalString.length;
        NSMutableString *decimalValue = [NSMutableString stringWithString:[originalvalue stringByReplacingOccurrencesOfString:@"." withString:@""]];
        for (NSInteger index = 0; index < decimals - decimalLength; index ++) {
            [decimalValue appendString:@"0"];
        }
        return decimalValue;
    }else{
        NSMutableString *decimalValue = [NSMutableString stringWithString:originalvalue];
        for (NSInteger index = 0; index < decimals; index ++) {
            [decimalValue appendString:@"0"];
        }
        return decimalValue;
    }
...
/**
 去掉零 按精度
 @param decimalValue 编码后的
 @param decimals 精度
 @return 解码后数据
 */
+ (NSString *)uncodingWithDecimalValue:(NSString *)decimalValue Decimals:(NSInteger)decimals{
    
    if ([decimalValue doubleValue] == 0) {
        return @"0";
    }else{
        if (decimalValue.length < decimals) {
            NSMutableString *originalvalue = [NSMutableString stringWithString:decimalValue];
            for (NSInteger index = 0; index < decimals - decimalValue.length; index ++) {
                [originalvalue insertString:@"0" atIndex:0];
            }
            [originalvalue insertString:@"0." atIndex:0];
            return [self changeFloatWithString:originalvalue];
        }else if (decimalValue.length == decimals) {
            NSMutableString *originalvalue = [NSMutableString stringWithString:decimalValue];
            [originalvalue insertString:@"0." atIndex:0];
            return [self changeFloatWithString:originalvalue];
        }else{
            NSMutableString *originalvalue = [NSMutableString stringWithString:decimalValue];
            [originalvalue insertString:@"." atIndex:decimalValue.length - decimals];
            return [self changeFloatWithString:originalvalue];
        }
    }
}
...
/**
 ...
 @return 返回值
 */
+ (NSString *)keepDecimalWithValue:(NSString *)value Digits:(NSInteger)digits{
    //去掉无效0
    NSMutableString *returnValue = [NSMutableString stringWithString:[BYTools changeFloatWithString:value]];
    if ([returnValue containsString:@"."]) {
        //保留有效位数
        NSArray *data = [returnValue componentsSeparatedByString:@"."];
        if (data[1]) {
            NSString *decimal = [NSString stringWithFormat:@"%@",data[1]];
            NSInteger length = decimal.length;
            if (length <= digits) {
                return returnValue;
            }else{
                [returnValue deleteCharactersInRange:NSMakeRange(returnValue.length - (length - digits), length - digits)];
                return [BYTools changeFloatWithString:returnValue];
            }
        }else{
            return returnValue;
        }
